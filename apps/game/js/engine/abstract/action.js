      ...
      if (typeof effect === 'object' && effect !== null && 'op' in effect) {
        ...
        switch (effect.op) {
          ...
          case 'set':
            ...
            break;
          case 'max':
            next = Math.max(current, effect.value);
            break;
          case 'min':
            next = Math.min(current, effect.value);
            break;
          default:
            continue;
        }
        // tuning-v6 2026-06-01: 数据驱动可选边界。声明式 op:add/multiply 此前无上限，
        // 导致 faction-actions 反复 +stability 累加溢出（稳定度涨到 3000+，应是 0-100）。
        // effect 可声明 { op, value, min, max }，仅对数值结果钳制；缺省不限（其他 key 不受影响）。
        if (typeof next === 'number') {
          if (typeof effect.max === 'number') next = Math.min(next, effect.max);
          if (typeof effect.min === 'number') next = Math.max(next, effect.min);
        }
        state.set(key, next);
      } else {
