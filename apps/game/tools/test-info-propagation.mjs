 ...
 * 验证点：
 *   1) 默认配置启用新闻/机会/觊觎链路。
 *   2) 显式 enabled=false 回退时不产生任何新闻/机会/觊觎事件。
 *   3) 激活态下：事件→新闻→传播→NPC 知晓；机会点生成；怀璧其罪暴露与抢夺/放过可触发。
 *
...
console.log('6) 默认启用态（产生信息系统事件）');
{
  ...
  ok(engine.tickManager.infoSystem.enabled, '默认配置下信息传播系统已启用');
  ok(engine.tickManager.opportunitySystem.enabled, '默认配置下机会点系统已启用');
  ok(infoCount > 0, `默认启用态下产生信息系统专属事件（实际 ${infoCount}）`);
}

// ── 集成：显式禁用态不产生信息事件 ───────────────────────
console.log('7) 显式禁用态（不产生信息系统事件）');
{
  const configs = baseConfigs();
  configs.worldNews = { ...configs.worldNews, enabled: false };
  configs.worldOpportunities = { ...configs.worldOpportunities, enabled: false };
  configs.balanceReward = { ...configs.balanceReward, enabled: false };
  configs.balanceCovet = { ...configs.balanceCovet, enabled: false };
  configs.seed = 1337;
  const engine = new WorldEngine();
  engine.init(configs);
  let infoCount = 0;
  ...
  ok(infoCount === 0, `显式禁用态下无任何信息系统专属事件（实际 ${infoCount}）`);
}
