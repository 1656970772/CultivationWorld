...
console.log('1) NPC 快照提供详细状态面板所需字段');
{
  ...
  assert.equal(typeof npc.insight, 'number', '包含游历感悟');
  assert.equal(typeof npc.totalProgress, 'number', '包含突破总进度');
  assert.equal(typeof npc.actionRemaining, 'number', '包含剩余行动天数');
  assert.equal(typeof npc.cultivationCap, 'number', '包含闭关进度上限');
  assert.equal(typeof npc.minCultivationRatio, 'number', '包含最低闭关占比');
  assert.equal(typeof npc.maxInsight, 'number', '包含游历感悟上限');
