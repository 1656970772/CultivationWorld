...
6. **配置化覆盖范围**：实施计划和代码设计中，势力、人物、地形、初始化资源、初始化道具列表、事件规则、规则参数、权重、模板数据等可配置内容默认不得硬编码，应沉淀到项目数据配置或对应策略/模板机制中。
7. **文件结构见 `docs/architecture/file-structure.md`。** 新增代码文件时需遵循已定义的目录结构。
...
4. **小说原文 `.txt` 已统一为 UTF-8 无 BOM。** 对 `docs/世界观参考/<作品>/<作品>.txt` 执行原文检索时默认按 UTF-8 读取，可直接使用 `rg -n "<关键词>" "<原文路径>"`；旧分析文档中提到的 GB18030/GBK 仅代表历史检索方式，不再作为当前原文编码规则。
...
3. **原文小说 `.txt` 的关键字检索默认按 UTF-8 处理。** `docs/世界观参考/<作品>/<作品>.txt` 已统一为 UTF-8 无 BOM；涉及历史分析文档中的 GB18030/GBK 说明时，将其视为旧检索记录，不要继续作为当前读取参数。
