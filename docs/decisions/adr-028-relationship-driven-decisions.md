...
ADR-027（第一期）建立了世界级 `RelationshipSystem` 单一真相源——随事件维护人际/人妖/妖妖三层关系边、衰减、可视化、存档，但**明确不驱动决策**。第一期遗留两项最高优先待办（见 `relationship-todo.md`）：
...
- **范围**：`relationship-todo.md` 第 1 项（关系驱动 Goal/行为）+ 第 6 项（妖群/领地自动建边）。其余（信任度/背叛、隐秘关系暴露、灵宠养成、化形建势力、跨图重建）仍留待后续。
...
- `collectExtraGoals` 末尾追加 `_buildRelationshipGoals(worldContext)`：`goalsEnabled` 关或无 qualifying 边时返回 `null`（**零漂移**：无合格关系即不产出，等价主路径）。仿机会 Goal **单点锁定**（按优先级取最高一个，写 `state.targetRelationshipId`）：
...
- `_initActions` 默认池补入 `act_npc_hunt_enemy`/`act_npc_kill_enemy`/`act_npc_assist_ally`/`act_npc_visit_benefactor`（均由关系 Goal/复仇派生状态 gate，零漂移）。
...
- 默认开但严格零漂移：无 qualifying 关系边时不产出 Goal（等价主路径）；`goalsEnabled=false` 完整回退一期。
...
- 回归：`test-relationship.mjs`、`test-revenge.mjs`、`test-goal-equivalence.mjs`（400 用例零漂移）、`test-obsession.mjs`、`test-bt.mjs`、`test-utility.mjs`、`test-memory.mjs` 全绿。
- `test-goap-golden.mjs` 指纹 `16c7c409`（新增 2 个关系行为后的新基线；`test-goal-equivalence` 400 用例确认现有目标规划路径零漂移——指纹变化仅因行为数+键集合变化导致 PRNG 采样错位，属"新增行为数据后应重新基线"的预期变更，参考 ADR-023）。
- `node apps/game/tools/simulate-analysis.mjs --days=200` 默认跑确认无报错、妖群/领地边涌现（实测 pack_member=314 / pack_leader=1 / territory_threat=95）；`RELATIONSHIP_GOALS_ACTIVE=0` 确认 pack_member=0 / pack_leader=0 / territory_threat=0（人际边不受影响，零漂移回退）。
