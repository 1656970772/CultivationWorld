...
5. `cultivationProgressRatio`、`cultivationProgress`、`insight`、`totalProgress` 仅作为兼容或派生字段；UI 和文档主语义使用“修为”。真气 `qi` 保留为独立资源，突破同时检查修为与真气。
...
- 旧字段和旧 executor 在迁移期需要兼容；删除前必须确认默认行为集、配置加载、生产引用和真实模拟都已走新路径。
