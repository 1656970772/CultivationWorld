...
状态：已采纳并实施（2026-06-03）。默认关闭不改变既有行为（摘要 `1169158b` 不变），多种子长程模拟通过。
...
- `data/definitions/macro-resources.json 或 data/items/`（416 行）混入了：货币（灵石 currency）、势力宏观资源（粮食 supply / 弟子 population）、炼丹炼器材料（灵药/灵矿/妖丹/妖材 material）、**重复的丹药（consumable）**、符箓、阵旗、储物袋、功法秘籍。
- `data/items/` 混入：材料、丹药（pill）、法宝（artifact）、符（talisman）。
...
所有可被 NPC / 势力持有、消耗、产出、转移的物品归入 `data/items/`，用 `category` 区分语义：
...
被经济/势力 AI/任务/妖兽掉落引用的 ID（灵石、`food`、`disciples`、`spirit_herb`、`ore`、`monster_core(_gN)`、`beast_material(_gN)`）**全部沿用原 ID**，迁移仅换文件不改名，保证默认关闭不改变既有行为。
...
**默认不自动触发**：本期只让「灵石可服用」这一能力存在，不新增自动消耗逻辑（无行为驱动则不发生），因此对现有模拟零影响——摘要 `1169158b` 不变。
...
- **默认关闭不改变既有行为验证**：`verify-determinism`（摘要 `1169158b` 不变）、`verify-effect-reuse`（新增灵石服用断言：low +1 / mid +120 复用 `ge_add_qi`）、`verify-gas-combat`（3 种子 × 800 天真实长程，锁血/遁地生效，无回归）。

## 实现位置

- 数据：`data/items/`（统一物品）、`data/definitions/macro-resources.json`（势力宏观资源，新建）；删除 `data/definitions/macro-resources.json 或 data/items/`。
