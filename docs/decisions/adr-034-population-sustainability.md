...
- `enabled=false` 时退化为原均匀选取（零漂移可回退）。
...
- 备份 `docs/balance/backup/pre-tuning-v3-social.json`、`pre-tuning-v4-quest-templates.json`、`pre-tuning-v4-npc-action-utils.js`、`pre-tuning-v4-combat.json`、`pre-tuning-v4-faction-ai-service.js`。
...
- 三机制均可一键回退，风险可控；GOAP 黄金指纹保持 `5740e12a` 零漂移。

## 验证

KPI 三轮对比（5000 天全激活态，详见 `docs/balance/tuning-2026-06-01-v3-result.md` / `tuning-2026-06-01-v4-result.md`）：
...
- GOAP 黄金指纹 `5740e12a` 在 v3/v4 均零漂移。
- 回归测试通过：`test-goap-golden` / `test-quest-reward-economy` / `test-monster-resource-loop` / `test-relationship-goals` / `test-revenge`。
...
- v3/v4 验证报告 `docs/balance/tuning-2026-06-01-v3-result.md`、`tuning-2026-06-01-v4-result.md`。
- ADR-015（宗门资源真相源统一 + 晋升体系）——`disciples` 抽象资源的来源背景。
- ADR-026（妖兽资源化闭环）——猎妖任务死亡的链路来源。
- 体检报告 `docs/reports/2026-05-30-项目分析与下一步建议.md`（P2：高层流动/淘汰偏弱）。
