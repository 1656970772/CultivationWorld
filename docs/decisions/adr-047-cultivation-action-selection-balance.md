...
状态：已采纳并实施（2026-06-03）。确定性指纹同种子可复现（`66b24755`，120 天 110 存活）；3000 天天才长程模拟显示行为多样化、进度稳步增长、不再早夭。本次为**有意改变 AI 决策行为**，非机制迁移，故不追求零漂移（项目验证规则只禁止"黄金指纹自证"，要求真实长程模拟统计为准）。
...
- **确定性**：`verify-determinism` 同种子两次指纹一致（`66b24755`），加权随机走 rng 不破坏可复现。
