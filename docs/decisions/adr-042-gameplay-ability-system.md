...
- **锁血被"固定绑死"在妖兽攻击路径上**：PvP/夺权场景永远不会锁血，遁地符即便实现也不会触发（见 [docs/TODO-combat-survival.md](../TODO-combat-survival.md) 阶段 2B 前置缺口）。
...
- 确定性必须保持（走 `worldContext.rng`/实体 `_rng`，不引入新 `Math.random`，ADR-038）。黄金指纹仅作重构期零漂移校验，**不作日常关卡**；新功能由数据开关控制可回退；以真实多种子长程模拟统计验证为准，禁止黄金指纹式作弊（AGENTS.md 验证规则）。
...
3. **阶段2（已完成 2026-06-03）**：现有功能逐项机制化迁移，每项数据开关可回退、迁移后零漂移校验。五项落地如下，**全程指纹 `1169158b` 不变（开/关开关均一致）**：
   ...
   - **突破回满血特效**（`apps/game/data/effects/cultivation-effects.json` `effect_breakthrough_full_heal`）：持 `Trait.BreakthroughFullHeal` Tag（特殊功法/体质授予）的实体突破成功时回满血（`hp override → maxHp`），由 `NPCEntity.tryBreakthroughFullHeal` 在 `npc-lifecycle` 突破成功后调用。默认无 NPC 持该 Tag，故默认不触发（零漂移）。
...
4. **阶段3（已完成 2026-06-03）**：见下"验证方式"。多种子长程模拟（3 种子 × 800 天）统计正常，开/关开关指纹一致证明零漂移，文档/数据规则已同步。

## 验证方式（遵守 AGENTS.md 验证规则）

- **禁止黄金指纹验证**：不给天才/特定对象开免疫、不隔离妖兽、不造一次性作弊脚本自证。
- 重构期黄金指纹仅作**零漂移校验工具**：阶段1 默认开关下行为应与 ADR-041 阶段1 基线一致（锁血逻辑等价迁移）。
...
- **确定性**：`tools/verify-determinism.mjs`（GAS 激活）同种子双跑指纹一致（`1169158b`），不同种子相异 —— 锁血/遁地全程走 `worldContext.rng`/实体 `_rng`，无确定性漂移。
- **阶段2 迁移零漂移（真实等价证明，非黄金指纹）**：把 `traitEffects.enabled` 与 `pillEffects.enabled` 两开关**关闭**（回退旧的直接读 config / 直接 state.set 路径）后再跑确定性校验，指纹**仍为 `1169158b`** —— 与开关开启（走 AttributeSet/EffectEngine）完全一致，证明灵根/体质加成、丹药结算、world-rules 聚合三项迁移在新旧两条路径上数值严格等价，开关可双向回退。
...
1. **遁地符落点缺陷（机制修正）**：原 `escape_teleport` 执行器是**全图随机**落点（且误用默认地图尺寸 100），会把残血遁走的低阶修士随机扔到危险区——天才第 5 天锁血+遁地成功脱险后被甩到陌生坐标，残血状态下第 6 天即撞上 grade3 妖兽**碾压**（orderGap≥40 跳过锁血）二次致死，使"保命符反而害命"。与 line 68"瞬移到安全位"的设计意图相悖。
   ...
   - **验证**：`verify-determinism` 同种子双跑指纹一致（`183b5fe3`，因落点逻辑变更轨迹故较旧 `1169158b` 改变，属预期行为变更而非回归）；`verify-gas-combat` 3 种子×800 天全 OK，锁血合计 42 次、遁地 4 次于真实模拟生效，PvP/攻战四场景统一管线无回归。
