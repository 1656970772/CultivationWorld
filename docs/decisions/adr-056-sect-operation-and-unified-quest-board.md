...
门派架构与运行模拟进入实现前，需要先把组织边界、任务板归属、配置清单、资源真相源和校验入口固定下来。此前宗门月俸、贡献、任务、库存和势力资源分散在势力、NPC、经济、任务和世界 Tick 逻辑中，容易在新增机制时回到硬编码分支或双轨 fallback。

本决策要求后续实现直接进入最新正式流程：门派仍是势力体系中的一种组织形态，门派运行只负责宗门内部运营，任务板作为通用任务域供门派、悬赏阁、坊市、事件和个人委托等来源复用。
...
- 后续实现不新增 `SectEntity`，避免势力系统和门派系统产生平行实体生命周期。
...
- 缺失配置在启动期暴露，后续任务不用维护旧月俸 fallback 或默认关闭 gate。
...
- 验证不得使用指纹或摘要一致性，应观察配置加载、校验报错、任务生成、库存变化、托管结算和真实模拟行为。
