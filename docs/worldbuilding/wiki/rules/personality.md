...
- **第一阶段落地：野心 → 晋升**。野心越高，`need_npc_ambition`（晋升需求）优先级越高；当弟子进入本境 `late` 或 `perfection` 小层时晋升需求进一步抬升，从而压过修炼需求，驱动其发起"挑战上位"（`act_npc_challenge`）。体现"先修为、后争位"的修仙逻辑。
...
- 性格不进入 GOAP 状态键（存于 state 的专用字段，不污染规划），故不影响行为的确定性回归（golden test 摘要不变）。
