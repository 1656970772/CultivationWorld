...
│   ├── memory.json           # 记忆系统（ADR-019/029）：事件→强度/衰减/恩怨增量（含 master_lost 恩师陨落）
│   ├── obsession.json        # 执念系统（ADR-019/020/029）：先天 roll + 后天记忆触发(含 master_lost→revenge) + goalState + goalMult(执念乘子) + seizeDisciple(夺舍触发参数)
